...
export default function AnimeDetails() {
            ...
            try {
                if (id) {
                    const [infoRes, epsRes, malDataRes] = await Promise.all([
                        fetch(`http://127.0.0.1:3005/anime/zoro/info/${id}`).catch(() => null),
                        fetch(`http://127.0.0.1:3005/anime/zoro/episodes/${id}`).catch(() => null),
    ...
    const handlePlayEpisode = async (episode: Episode, forceMode?: 'sub' | 'dub') => {
        ...
        try {
            const backendUrl = `http://127.0.0.1:3005/anime/zoro/watch/${encodeURIComponent(episode.id)}?lang=${modeToUse}`;
