...
export default function Home() {
    ...
    return (
        <main className="w-full flex flex-col">
            <section ref={heroRef} className="relative w-full h-[480px] xl:h-[520px]">
                {loading ? (
                    <div className="absolute inset-0 flex items-center justify-center bg-bg">
                        <div className="w-8 h-8 border-2 border-accent border-t-transparent rounded-full animate-spin"></div>
                    </div>
                ) : activeAnime && (
                    <HeroBanner anime={activeAnime} currentIndex={currentIndex} total={Math.min(trending.length, 20)} onNext={handleNext} onPrev={handlePrev} onSelect={handleSelectSlide} />
                )}
            </section>

            <div className="flex flex-col lg:flex-row gap-6 px-6 py-6 max-w-[1500px] mx-auto w-full">
                <section className="flex-1 min-w-0">
                        ...
                        <div className="mb-8" data-reveal>
                            <div className="flex items-center justify-between mb-5">
                                ...
                                <div className="flex items-center gap-1.5">
                                    <button onClick={() => { const el = document.getElementById('continue-scroll'); if (el) el.scrollBy({ left: -320, behavior: 'smooth' }); }} className="w-8 h-8 flex items-center justify-center rounded-xl bg-surface border border-border text-muted hover:text-fg hover:border-muted transition-all cursor-pointer">
                            ...
                            <div id="continue-scroll" className="flex gap-4 overflow-x-auto py-1 scrollbar-hide">
                                {history.map((item) => {
                                    const progressPercent = item.duration > 0 ? (item.progress / item.duration) * 100 : 0;
                                    return (
                                        <Link key={`history-${item.anime_id}`} to={`/anime/${item.anime_id}?ep=${item.episode_id}`} className="group w-[260px] md:w-[280px] shrink-0 block">
                                            <div className="w-full aspect-video rounded-xl overflow-hidden bg-surface border border-border relative">
                                            ...
                                            <h3 className="text-xs font-medium text-muted mt-2 truncate group-hover:text-fg transition-colors">{item.anime_title}</h3>
                                        </Link>
                                    );
                                })}
                            </div>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-5 gap-y-8 gap-x-4">
                        {loading ? (
                            ...
                            schedule.slice(0, 20).map((anime, index) => (
                                <Link key={`latest-${anime.id}-${index}`} to={`/anime/${anime.id}?ep=${(anime as any).episodeId || (anime as any).episode || 1}`} className="group block" data-reveal data-reveal-delay={`${(index % 5) * 100}`}>
                                    <div className="aspect-[2/3] bg-surface rounded-xl overflow-hidden mb-2 border border-border relative shadow-sm">
                                        ...
                                        <span className="absolute top-2.5 right-2.5 bg-black/70 text-white text-[9px] font-semibold px-2 py-0.5 rounded-full uppercase tracking-wider">EP {(anime as any).episode || "1"}</span>
                                        <span className="absolute bottom-2.5 left-2.5 bg-black/70 text-white text-[9px] font-semibold px-2 py-0.5 rounded-full uppercase tracking-wider">{anime.subOrDub || "SUB"}</span>
                                    </div>
                                    <h4 className="font-semibold text-sm truncate text-fg group-hover:text-accent transition-colors leading-tight">{anime.title}</h4>
                                    <p className="text-[10px] text-muted font-medium mt-0.5 uppercase tracking-wider">TV Series</p>
                                </Link>
                            ))
                        )}
                    </div>
