...
app.get('/anime/zoro/top-airing', async (req, res) => {
  ...
  try {
    ...
    return res.json({ results: CACHE.trending || [] });
  }
});
...
app.get('/anime/zoro/info/:id', async (req, res) => {
  ...
  try {
    ...
    let relations = (anime.relations?.edges || []).filter(edge => ['PREQUEL', 'SEQUEL', 'ALTERNATIVE', 'SPIN_OFF', 'SIDE_STORY'].includes(edge.relationType)).map(edge => ({
      id: edge.node.id,
      title: edge.node.title?.english || edge.node.title?.romaji || `${edge.node.format || 'TV'} Entry`,
...
app.get('/anime/zoro/episodes/:id', async (req, res) => {
  ...
  try {
    const q = `query($id:Int){Media(id:$id){status format episodes nextAiringEpisode{episode}}}`;
    ...
    if (j?.data?.Media) {
      ...
      // 🔥 EPISODE COUNT FIX: We now prioritize 'nextAiringEpisode' BEFORE checking the total 'episodes' property.
      // This stops Anilist from returning 13 for currently airing shows when only 6 are out.
...
app.get('/proxy/stream', async (req, res) => {
  ...
  try {
    ...
    if (upstreamType.includes('audio/,') || upstreamType.includes('text/plain')) {
    ...
    nodeStream.pipe(res).on('error', () => { nodeStream.destroy(); });
...
// ==========================================
// 🛑 WATCH ROUTE (STRICT LOCALHOST MIRURO INTEGRATION)
// ==========================================
app.get('/anime/zoro/watch/:episodeId', async (req, res) => {
  ...
  if (episodeId.startsWith('allanime-')) {
    ...
    requestedAnimeId = parts[0].split('-vid-')[0].replace('allanime-', '');
    epNum = parts[1] || epNum;
  } else if (episodeId.startsWith('auto-')) {
    const parts = episodeId.split('-');
    requestedAnimeId = parts[1] || requestedAnimeId;
    epNum = parts[2] || epNum;
  } else if (episodeId.includes('-episode-')) {
    const parts = episodeId.split('-episode-');
    requestedAnimeId = parts[0];
    epNum = parts[1];
  } else {
  ...
  const MIRURO_API_BASE = 'http://127.0.0.1:8000';

  try {
    console.log(`[WATCH] Requesting episode list mappings from private Miruro microservice firmly locked at ${MIRURO_API_BASE} for ID: ${requestedAnimeId}...`);
    ...
    if (epListRes.ok) {
      ...
      for (const pKey of [...new Set(providerKeys)]) {
        ...
        if (matchedEp?.id) {
          console.log(`[WATCH] Testing provider '${pKey}' with path: ${matchedEp.id}`);
          ...
          try {
            ...
            if (streamRes.ok) {
              ...
              if (payload?.streams?.length > 0) {
                ...
                console.log(`[WATCH] Successfully locked stream on provider '${pKey}'!`);
                break;
              }
            }
          } catch (err) {
            console.warn(`[WATCH] Provider '${pKey}' failed to deliver manifest. Trying next...`);
          }
        }
      }
      ...
      if (extractedWatchPath && finalStreamPayload) {
        ...
        console.log(`[WATCH] ✅ High-fidelity manifest transport securely acquired via self-hosted API pipeline!`);
